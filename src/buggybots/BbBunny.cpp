...
BbBunny::BbBunny(VrGroup *pGroup) : VrConcaveMesh(pGroup) {
	vuInfo().setMaterialName("Torus/mat_2_0") ;
}
void BbBunny::doCreate(const BwPartInfo& info) {
	vuInfo().setMeshName(getName()) ;
	VrConcaveMesh::doCreate(info) ;
}
//
/////////////////
...
class BbBunnyBuilder {
public:
	BbBunnyBuilder(BbBunny *pModel) {
		p_model = pModel ;
	}
	void buildFaces() ;
	void buildFace(int faceIndex) ;
	void buildTexcoords(int faceIndex) ;
	void generateMesh() ;
	//Data Members
public:
	BbBunny *p_model ;
	VrOmTriMesh::VertexHandle h_vertices[VertexCount] ;
	VrOmTriMesh *p_mesh ;
};
//
void BbBunny::generateMesh() {
	VrTriMesh::generateMesh() ; //sets loader to create ode mesh from ogre mesh

	BbBunnyBuilder builder(this) ;
...
//////////////////////////////
//////////////////////
void BbBunnyBuilder::buildTexcoords(int faceIndex) {
	p_mesh->set_texcoord2D(h_vertices[Indices[faceIndex][0]], VrOmTriMesh::TexCoord2D(.49, .49));
	p_mesh->set_texcoord2D(h_vertices[Indices[faceIndex][1]], VrOmTriMesh::TexCoord2D(.25, .01));
	p_mesh->set_texcoord2D(h_vertices[Indices[faceIndex][2]], VrOmTriMesh::TexCoord2D(.01, .49));
}
void BbBunnyBuilder::buildFace(int faceIndex) {
	std::vector<VrOmTriMesh::VertexHandle>  face_vhandles;

	face_vhandles.push_back(h_vertices[Indices[faceIndex][0]]);
	face_vhandles.push_back(h_vertices[Indices[faceIndex][1]]);
	face_vhandles.push_back(h_vertices[Indices[faceIndex][2]]);

	p_mesh->add_face(face_vhandles) ;

	buildTexcoords(faceIndex) ;
}
//const int VertexCount = 453;
//const int IndexCount = 902 * 3;
void BbBunnyBuilder::buildFaces() {
	for( size_t i = 0 ; i < VertexCount ; i++) {
		h_vertices[i] = p_mesh->add_vertex(
			VrOmTriMesh::Point(Vertices[i*3], Vertices[i*3+1], Vertices[i*3+2])) ;
	}
	for( size_t i = 0 ; i < IndexCount/3 ; i++) {
		buildFace(i) ;
	}
}
void BbBunnyBuilder::generateMesh() {
